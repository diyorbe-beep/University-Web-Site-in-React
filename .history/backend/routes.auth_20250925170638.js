...
router.get('/me', async (req, res) => {
	try {
		...
		// Find user based on role and session
		// For now, we'll use a simple approach - store user info in session or find by role
		// Since we don't have proper JWT payload, we'll need to track logged in user differently
		
		// Get the last logged in user of this role from a simple session store
		const sessionKey = `lastUser_${userRole}`;
		let userId = req.session?.[sessionKey];
		
		// If no session, find the most recent user of this role
		if (!userId) {
			const allUsers = await users.all();
			const usersOfRole = allUsers.filter(u => u.role === userRole);
			if (!usersOfRole.length) {
				return res.status(404).json({ error: 'No users found' });
			}
			// Get the most recently created user of this role
			const sortedUsers = usersOfRole.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
			userId = sortedUsers[0]._id;
		}
