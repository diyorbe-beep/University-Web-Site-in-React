...
export default function Admin() {
  ...
  async function deleteAllFiltered() {
    ...
    const confirmMessage = `${ordersToDelete.length} ta buyurtmani o'chirmoqchimisiz? Bu amalni qaytarib bo'lmaydi.`;
    ...
    try {
      ...
      setOrders((prev) => prev.filter((o) => !ordersToDelete.some(del => del._id === o._id)));
    } catch (error) {
      alert('Ba\'zi buyurtmalarni o\'chirishda xatolik yuz berdi');
    }
  }
    ...
    return (
      <div className="admin-wrapper">
        <div className="admin-card admin-login">
          ...
          <div className="admin-hint">Agar admin yo'q bo'lsa, backend: POST /auth/seed-admin</div>
        </div>
      </div>
    );
  }
            ...
            <div className="orders-controls">
              ...
              {filteredOrders.length > 0 && (
                <button
                  ...
                  title={`${filteredOrders.length} ta buyurtmani o'chirish`}
                >
                  🗑️ Barchasini o'chirish ({filteredOrders.length})
                </button>
              )}
            </div>
          </div>
